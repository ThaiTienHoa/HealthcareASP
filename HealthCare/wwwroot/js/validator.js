// validator custom, project đang dùng có sẵn của jquery
// Đối tượng Validator
...
Validator.minLength = function (selector, min, message){
    return{
        selector :selector,
        test: function(value){
            return value.length >= min ? undefined : message || `Mật khẩu quá ngắn (ngắn nhất là ${min} ký tự)`
        }
    };
}
...
Validator.isPhone = function (selector, message) {
    return {
        selector: selector,
        test: function (value) {
            /*0123456789 hoặc (+84)123456789*/
            var regex = /^(0\d{9})|(\(\+\d{2,3}\)\d{9})$/
